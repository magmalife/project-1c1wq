...
async def setup_save(
    ...
    if hevyless_username:
        config["hevyless_username"] = hevyless_username
            ...
            if hasattr(_db, '_get_conn'):
                ...
                with _db._get_conn() as conn:
                    with conn.cursor() as cur:
                        ...
                        if config.get("hevyless_username"):
                            cur.execute("""
                                INSERT INTO app_cache (key, value)
                                VALUES ('hevyless_username', %s)
                                ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value
                            """, (_json.dumps(config["hevyless_username"]),))
                    conn.commit()
        except Exception as e:
...
async def settings_save(
    request: Request,
    hevy_api_key: str = Form(""), garmin_email: str = Form(""), garmin_password: str = Form(""), hevyless_username: str = Form(""),
    ...
    if db.get_database_url():
        try:
            ...
            if config.get("hevyless_username"):
                _db.set_app_config("hevyless_username", config["hevyless_username"])
...
async def api_toggle_autosync(request: Request):
    ...
    try:
        ...
        interval = 120
    if interval not in (1, 30, 60, 120, 240, 360, 720, 1440):
        interval = 120
...
async def cron_sync(request: Request):
    ...
    if not auto_cfg.get("enabled"):
        return JSONResponse({"status": "disabled", "message": "Auto-sync is disabled"})

    # Check if enough time has passed based on selected interval
    now = time.time()
    last_run = auto_cfg.get("_last_cron_run", 0)
    interval_minutes = auto_cfg.get("interval_minutes", 120)
    interval_seconds = interval_minutes * 60

    # allow 30s drift in case cron triggers slightly early
    if now - last_run < interval_seconds - 30:
        return JSONResponse({"status": "skipped", "message": f"Waiting for next interval ({interval_minutes}m)"})

    auto_cfg["_last_cron_run"] = now
    from datetime import datetime, timezone
    auto_cfg["last_sync"] = datetime.now(timezone.utc).isoformat()
    config["auto_sync"] = auto_cfg
    save_config(config)

    # Persist to database so Vercel remembers the last run
    if db.get_database_url():
        try:
            import json as _json
            _db = db.get_db()
